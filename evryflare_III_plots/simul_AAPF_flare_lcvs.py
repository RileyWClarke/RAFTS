...
evry_response=[]
tess_response=[]
with open("four_lines_aapf.csv","r") as INFILE:
    for lines in INFILE:
        _lambda.append(float(lines.split(",")[0]))
        flux30.append(float(lines.split(",")[1]))
        flux10.append(float(lines.split(",")[2]))
        evry_response.append(float(lines.split(",")[3]))
        tess_response.append(float(lines.split(",")[4]))
_lambda=np.array(_lambda)
flux30=np.array(flux30)
flux10=np.array(flux10)
evry_response=np.array(evry_response)
tess_response=np.array(tess_response)
...
plt.xlabel("Time [min]",color="black",fontsize=24)
plt.ylabel("$\Delta g^{\prime}$-mag",color="royalblue",fontsize=24)
...
plt.yticks(color="firebrick",fontsize=20)
plt.xticks(color="black",fontsize=20)
plt.ylabel("$\Delta$TESS mag",color="firebrick",fontsize=24)

plt.text(13,-0.67,"HD 270712,\n2018-10-20 5:36 UT", fontsize=20)
...
plt.text(38.5,-0.24, "Supplemental\nTESS data",color="firebrick",fontsize=20)
...
plt.text(32.5,-0.49, "Evryscope data",color="royalblue",fontsize=20)
...
ax3.plot(_lambda, evry_response, color="lightgrey",linewidth=3)
#ax3.plot(_lambda, tess_response, color="lightgrey",linewidth=2)

ax3.plot(np.linspace(373.5,1123.0,num=100), np.zeros(100), ls="-", color="white",linewidth=3.0)


ax3.plot(_lambda, flux30, color="mediumslateblue",linewidth=3)
ax3.plot(_lambda, flux10, color="darkorange",linewidth=3.3)

ax3.text(585,0.35,"Evryscope\n($g^{\prime}$)",color="grey",fontsize=24,alpha=0.75)
#ax3.text(940,0.93,"TESS",color="grey",fontsize=14,alpha=0.75)

ax3.text(144,0.81,"30,000 K",color="mediumslateblue",fontsize=24)
ax3.text(760,0.037,"9000 K",color="darkorange",fontsize=24)
plt.ylabel("Flux [norm]",fontsize=24)
plt.xlabel("Wavelength [nm]",fontsize=24)

ax3.plot(np.linspace(1100,1250,num=100), np.zeros(100), ls="-", color="white",linewidth=5.0)
...
plt.tight_layout()
plt.savefig("/home/wshoward/Desktop/simulflare_AAPF.png")
plt.show()
